...
# Script flow:
# 1. Decide which folder the config should point at.
# 2. Decide where the output text file should be written.
# 3. Save those values into config.psd1 with beginner-friendly comments.
...
"@
	...
	Write-Host "SourceFolder = $storedTargetFolder"
	Write-Host "OutputPath = $storedOutputPath"
	Write-Host "ExistingOutputFileMode = $ExistingOutputFileMode"
	Write-Host "IncludeSubfolders = $($IncludeSubfolders.IsPresent.ToString().ToLower())"
}
catch {
	Write-Error $_
	exit 1
