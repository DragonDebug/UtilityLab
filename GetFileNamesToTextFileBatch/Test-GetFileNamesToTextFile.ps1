...
function Write-TestConfig {
	...
	return Write-BatchTestConfig -ToolRoot $ToolRoot -Pairs @($pair)
}

function Write-BatchTestConfig {
	param(
		[Parameter(Mandatory = $true)]
		[string]$ToolRoot,
	...
	$pairBlocks = foreach ($pair in $Pairs) {
		$pairSourceFolder = $pair.SourceFolder.Replace("'", "''")
		$pairOutputPath = $pair.OutputPath.Replace("'", "''")
		$pairMode = $pair.ExistingOutputFileMode.Replace("'", "''")
		$pairIncludeSubfolders = ([bool]$pair.IncludeSubfolders).ToString().ToLower()
		$pairAllowReparsePointSourceRoot = ([bool]$pair.AllowReparsePointSourceRoot).ToString().ToLower()
		$pairAllowReparsePointOutputDirectory = ([bool]$pair.AllowReparsePointOutputDirectory).ToString().ToLower()

		@"
		@{
			SourceFolder = '$pairSourceFolder'
			OutputPath = '$pairOutputPath'
			ExistingOutputFileMode = '$pairMode'
			IncludeSubfolders = `$$pairIncludeSubfolders
			AllowReparsePointSourceRoot = `$$pairAllowReparsePointSourceRoot
			AllowReparsePointOutputDirectory = `$$pairAllowReparsePointOutputDirectory
		}
"@
	}
...
try {
	...
	Assert-Equal -Actual $configAfterSetup.Pairs.Count -Expected 1 -Message "Case 7 setup script should write one starter pair."
	Assert-Equal -Actual $configAfterSetup.Pairs[0].SourceFolder -Expected $case7Target -Message "Case 7 setup script wrote the wrong source folder."
	Assert-Equal -Actual $configAfterSetup.Pairs[0].OutputPath -Expected (Join-Path $case7Target "file_names.txt") -Message "Case 7 setup script wrote the wrong output path."
	Assert-Equal -Actual $configAfterSetup.Pairs[0].ExistingOutputFileMode -Expected 'CreateNew' -Message "Case 7 setup script wrote the wrong existing-file mode."
	Assert-Equal -Actual ([bool]$configAfterSetup.Pairs[0].IncludeSubfolders) -Expected $true -Message "Case 7 setup script wrote the wrong recursion value."
	...
	# Test 13: one run can write multiple outputs from multiple sources.
	$case13Root = Join-Path $tempRoot "case13"
	New-Item -ItemType Directory -Path $case13Root | Out-Null
	$case13Tool = New-TestHarness -RootPath $case13Root
	$case13SourceA = Join-Path $case13Root "source-a"
	$case13SourceB = Join-Path $case13Root "source-b"
	$case13NestedB = Join-Path $case13SourceB "nested"
	New-Item -ItemType Directory -Path $case13SourceA | Out-Null
	New-Item -ItemType Directory -Path $case13NestedB -Force | Out-Null
	Set-Content -LiteralPath (Join-Path $case13SourceA "alpha.keep") -Value "a" -Encoding UTF8
	Set-Content -LiteralPath (Join-Path $case13NestedB "beta.keep") -Value "b" -Encoding UTF8
	$case13OutputA = Join-Path $case13Root "alpha.txt"
	$case13OutputB = Join-Path $case13Root "beta.txt"
	Write-BatchTestConfig -ToolRoot $case13Tool -Pairs @(
		@{
			SourceFolder = $case13SourceA
			OutputPath = $case13OutputA
		...
		@{
			SourceFolder = $case13SourceB
			OutputPath = $case13OutputB
	...
	$case13Run = Invoke-PowerShellFile -ScriptPath (Join-Path $case13Tool "GetFileNamesToTextFile.ps1")
	Assert-Equal -Actual $case13Run.ExitCode -Expected 0 -Message "Case 13 failed."
	Assert-Equal -Actual ((Get-Content -LiteralPath $case13OutputA -Raw).Trim()) -Expected "alpha.keep" -Message "Case 13 wrote the wrong first output file."
	Assert-Equal -Actual ((Get-Content -LiteralPath $case13OutputB -Raw).Trim()) -Expected "nested\beta.keep" -Message "Case 13 wrote the wrong second output file."

	# Test 14: one failing pair does not stop later pairs from running.
	$case14Root = Join-Path $tempRoot "case14"
	New-Item -ItemType Directory -Path $case14Root | Out-Null
	$case14Tool = New-TestHarness -RootPath $case14Root
	$case14SourceA = Join-Path $case14Root "source-a"
	$case14SourceC = Join-Path $case14Root "source-c"
	New-Item -ItemType Directory -Path $case14SourceA | Out-Null
	New-Item -ItemType Directory -Path $case14SourceC | Out-Null
	Set-Content -LiteralPath (Join-Path $case14SourceA "first.keep") -Value "a" -Encoding UTF8
	Set-Content -LiteralPath (Join-Path $case14SourceC "third.keep") -Value "c" -Encoding UTF8
	$case14MissingSource = Join-Path $case14Root "missing-source"
	$case14OutputA = Join-Path $case14Root "first.txt"
	$case14OutputB = Join-Path $case14Root "missing.txt"
	$case14OutputC = Join-Path $case14Root "third.txt"
	Write-BatchTestConfig -ToolRoot $case14Tool -Pairs @(
		@{
			SourceFolder = $case14SourceA
			OutputPath = $case14OutputA
			IncludeSubfolders = $false
			AllowReparsePointSourceRoot = $true
			AllowReparsePointOutputDirectory = $true
			ExistingOutputFileMode = 'Overwrite'
		...
		@{
			SourceFolder = $case14MissingSource
			OutputPath = $case14OutputB
			IncludeSubfolders = $false
			AllowReparsePointSourceRoot = $true
			AllowReparsePointOutputDirectory = $true
			ExistingOutputFileMode = 'Overwrite'
		...
		@{
			SourceFolder = $case14SourceC
			OutputPath = $case14OutputC
			IncludeSubfolders = $false
			AllowReparsePointSourceRoot = $true
			AllowReparsePointOutputDirectory = $true
			ExistingOutputFileMode = 'Overwrite'
	...
	$case14Run = Invoke-PowerShellFile -ScriptPath (Join-Path $case14Tool "GetFileNamesToTextFile.ps1")
	if ($case14Run.ExitCode -eq 0) {
		throw "Case 14 should return a failing exit code when any pair fails."
	}
	if (-not (Test-Path -LiteralPath $case14OutputA -PathType Leaf)) {
		throw "Case 14 should still create the first valid output file."
	}
	if (-not (Test-Path -LiteralPath $case14OutputC -PathType Leaf)) {
		throw "Case 14 should still create the later valid output file."
	}
	if ($case14Run.StdErr -notmatch 'Pair 2 failed') {
		throw "Case 14 should report which pair failed. StdErr: $($case14Run.StdErr)"
	}

	# Test 15: duplicate output paths are rejected before writing.
	$case15Root = Join-Path $tempRoot "case15"
	New-Item -ItemType Directory -Path $case15Root | Out-Null
	$case15Tool = New-TestHarness -RootPath $case15Root
	$case15SourceA = Join-Path $case15Root "source-a"
	$case15SourceB = Join-Path $case15Root "source-b"
	New-Item -ItemType Directory -Path $case15SourceA | Out-Null
	New-Item -ItemType Directory -Path $case15SourceB | Out-Null
	Set-Content -LiteralPath (Join-Path $case15SourceA "alpha.keep") -Value "a" -Encoding UTF8
	Set-Content -LiteralPath (Join-Path $case15SourceB "beta.keep") -Value "b" -Encoding UTF8
	$case15Output = Join-Path $case15Root "shared.txt"
	Write-BatchTestConfig -ToolRoot $case15Tool -Pairs @(
		@{
			SourceFolder = $case15SourceA
			OutputPath = $case15Output
			IncludeSubfolders = $false
			AllowReparsePointSourceRoot = $true
			AllowReparsePointOutputDirectory = $true
			ExistingOutputFileMode = 'Overwrite'
	...
	$case15Run = Invoke-PowerShellFile -ScriptPath (Join-Path $case15Tool "GetFileNamesToTextFile.ps1")
	if ($case15Run.ExitCode -eq 0) {
		throw "Case 15 should fail when multiple pairs target the same output file."
	}
	if ($case15Run.StdErr -notmatch 'Each\s+pair must use a unique OutputPath') {
		throw "Case 15 should report the duplicate output path error. StdErr: $($case15Run.StdErr)"
	}
